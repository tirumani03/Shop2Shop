...
$subcatg=$_REQUEST['view1'];
   ...
   $sel=mysqli_query($con,"select * from items where catg='$catg' and subcatg='$subcatg'");
	...
	if($c!=NULL)
	  {
		...
		foreach($c as $c1)
		  {
		  $sel=mysqli_query($con,"select * from items where dat='$c1'");
		  ...
		  $t=date("d-m-y h-i-s");
		   ...
		   mysqli_query($con,"insert into trash values('$catg','$subcatg','$img','$itemno','$price','$desc','$t')");
		    mysqli_query($con,"delete from items where dat='$c1'");
		   $flag=1;
		   }
		   if($flag==1)
		     {
